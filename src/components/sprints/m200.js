...
import { Sprint200m, Sprint200w } from '../../data/sprints-data';

const m100 = (props) => {
  let resultsM = Sprint200m.map((result) => {
  ...
  let resultsW = Sprint200w.map((result) => {
    return <SprintE pos={result.pos}
                   name={result.name}
                   country={result.country}
                   mark={result.mark}
  ...
  return (
  	<div>
  		<h4>How it works</h4>
  		<small>Runners race for 100m around a bend and 100m down the home straight. They start from blocks and run in lanes.</small>
  		<br/><br/>
